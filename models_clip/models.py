...
class PeftModelFromCLIP(nn.Module):
    def __init__(self, args, clip_model, num_classes):
        ...
        if args.arch.startswith("CLIP-ViT"):
            ...
            self.tuner = ViT_Tuner(args, clip_model.visual, num_classes)

        elif args.arch.startswith("CLIP-RN"):
            self.text_encoder = CLIP_Text(clip_model)
            self.image_encoder = Peft_RN(clip_model.visual)
            self.tuner = RN_Tuner(args, clip_model.visual, num_classes)
        ...
        self.head = CosineClassifier(feat_dim, num_classes, dtype)
