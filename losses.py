...
class VSLoss(nn.Module):
    ...
    def __init__(self, cls_num_list, gamma=0.3, tau=1.0, weight=None, reduction='mean'):
...
class LALoss(nn.Module):
    def __init__(self, cls_num_list, reduction='mean'):
        ...
        prior = [cls_num / sum(cls_num_list) for cls_num in cls_num_list]
        self.prior = torch.cuda.FloatTensor(prior)
