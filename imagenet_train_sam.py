...
from utils import *
from datasets.imagenet import ImageNetLTDataLoader
...
def main():
    ...
    name_list = [args.dataset, args.arch, args.loss_type, args.train_rule, args.SAM_type, str(args.rho), 'wd', str(args.weight_decay), 'sched', args.rho_schedule, 'rho_steps' , '-'.join(map(str, args.rho_steps)) ,'seed', str(args.seed), args.exp_str]
...
def main_worker(gpu, args):
    ...
    shot_list = np.load("./data/imagenet_lt_shot_list.npy")
    many_shot_classes = shot_list[0]
    medium_shot_classes = shot_list[1] 
    few_shot_classes = shot_list[2]

    args.head_class_idx = shot_list[0]
    args.med_class_idx = shot_list[1]
    args.tail_class_idx = shot_list[2]
    ...
    if args.dataset == 'imagenet':
        data_loader = ImageNetLTDataLoader(data_dir=args.data_path, batch_size=args.batch_size, num_workers=args.workers, shuffle=True)
...
def train(train_loader, model, criterion, optimizer, epoch, args, log, few_shot_class, medium_shot_class, many_shot_class, cls_num_list):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.SAM_type == "Focal-SAM":
            ...
            # output, loss = min_max_focal_fix_wsam_asam_step(args, model, criterion, input, target, optimizer, cls_num_list)
