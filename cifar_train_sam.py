...
from losses import LDAMLoss, FocalLoss, VSLoss, LALoss
from sam import SAM
...
from train_step import per_class_asam_step, split_k_sample_asam_step, split_k_class_asam_step, split_k_sample_fix_asam_step, split_shot_asam_step, split_k_sample_loss_asam_step, split_shot_wsam_asam_step, focal_wsam_asam_step, focal_fix_wsam_asam_step, focal_wsam_sample_asam_step, min_max_focal_fix_wsam_asam_step, imbsam_step, focal_sam_step
...
def main():
    ...
    name_list = [args.dataset, args.arch, args.loss_type, args.train_rule, args.imb_type, str(args.imb_factor), args.SAM_type, str(args.rho), 'sched', args.rho_schedule, 'seed', str(args.seed), args.exp_str]
...
def main_worker(gpu, args):
    ...
    if args.SAM_type == 'SAM':
        ...
        optimizer = FocalSAM(params=model.parameters(), base_optimizer=base_optimizer, rho=args.rho, adaptive=False,  lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.loss_type == 'CE':
            ...
            criterion = LDAMLoss(cls_num_list=cls_num_list, max_m=0.5, s=30, weight=per_cls_weights, reduction='none').cuda()
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, log_training, few_shot_class, medium_shot_class, many_shot_class, cls_num_list)
