...
from utils import *
from datasets.inaturalist_clip import iNaturalistDataLoader
...
parser.add_argument('--data_path', default='data/iNaturalist', type=str, metavar='PATH',
...
def main_worker(gpu, args):
    ...
    if args.dataset == 'inaturalist':
        data_loader = iNaturalistDataLoader(data_dir=args.data_path, batch_size=args.batch_size, num_workers=args.workers, shuffle=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, log_training, few_shot_classes, medium_shot_classes, many_shot_classes, cls_num_list, scaler)
...
def train(train_loader, model, criterion, optimizer, epoch, args, log, few_shot_class, medium_shot_class, many_shot_class, cls_num_list, scaler):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.SAM_type == "Focal-SAM":
            output, loss = focal_sam_step(args, model, criterion, input, target, optimizer, cls_num_list, scaler)
...
def validate(val_loader, model, criterion, epoch, args, log=None, tf_writer=None, flag='val'):
    ...
    with torch.no_grad():
        ...
        output_group = 'Head: %.3f, Med: %.3f, Tail: %.3f'%(head_acc, med_acc, tail_acc)

        if log is not None:
            log.write(output + '\n')
            log.write(out_cls_acc + '\n')
            log.write(output_group + '\n')
            log.flush()
